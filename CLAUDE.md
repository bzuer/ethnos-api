...
Academic bibliographic system API built with Node.js/Express, backed by MariaDB. Full-text search runs entirely against MariaDB FULLTEXT indexes (`ft_summary_pubs_content`, `ft_summary_pubs_metadata`) on `summary_publications` plus the `persons` / `signatures` tables. There is no Sphinx daemon, RT index, or external indexer — that legacy stack was removed.
...
- **No real-time indexing component is required.** Search is satisfied directly by MariaDB FULLTEXT indexes on `summary_publications`. The operator-side refresh procedure (`sp_refresh_summary_publications_for_work`) still rebuilds summary rows after writes to `publications` / `works` / `authorships` / `work_subjects` / `files`, and the FULLTEXT indexes (`ft_summary_pubs_content`, `ft_summary_pubs_metadata`) update in-place with the row.
- Legacy artefacts explicitly absent (do not reintroduce): `sphinx_works_summary`, `sphinx_venues_summary`, `sphinx_persons_summary`, `work_author_summary`, `work_subjects_summary`, `sphinx_queue`, `processing_log`, `person_match_log`, `staging_*`, `temp_*`, the four dormant `v_*` views, and any Sphinx runtime artefact (`publications_poc`, `publications_rt`, `persons_poc`, `venues_poc`, the `searchd` daemon, `sphinx-unified.conf`, `/var/lib/ethnos-api/sphinx`, `realTimeIndexing.service`, `sphinx*.service`).
...
- `src/services/` — 17 services (includes `publications`, `search`, `autocomplete`, `cache`, `homepageStats`).
...
- `config/` — `swagger.config.js` (no other files; no `.bak`).
...
- Endpoints wired: `/works`, `/works/showcase`, `/publications`, `/search/works`, `/search/advanced`, `/persons/{id}/works`, `/venues/{id}/works`. All paths translate the citation bounds to `sp.work_citation_count >= ?` / `sp.work_citation_count <= ?` clauses against `summary_publications`.
...
- Fulltext (`q`) and metadata filters (`venue_name`, `author`, `subject`) run as MariaDB FULLTEXT clauses against `summary_publications`. `q` uses `MATCH(sp.title_search, sp.abstract_search) AGAINST (? IN BOOLEAN MODE)` against `ft_summary_pubs_content`. Metadata filters use `MATCH(sp.authors_search, sp.venue_search, sp.subjects_search) AGAINST (? IN BOOLEAN MODE)` against `ft_summary_pubs_metadata` with every token of every filter forced to required (`+token1 +token2 …`). Joining the filter values with a plain space in BOOLEAN MODE would treat them as OR and return tens of thousands of false positives (e.g. `author=Pedro Luiz Cortés` once bloated to 29 468 unrelated rows); the per-token `+` prefix collapses that to the actual matches and keeps `subject=social anthropology` under the count budget (the strict-phrase form `+"…"` ran past the 6 s statement timeout on `ft_summary_pubs_metadata`).
...
- `/publications` and `/publications/{id}` are backed by `summary_publications`. Free-text queries (`q`) use `MATCH(sp.title_search, sp.abstract_search) AGAINST (? IN BOOLEAN MODE)` against `ft_summary_pubs_content`; metadata filters (`venue` / `author` / `subject`) use `MATCH(sp.authors_search, sp.venue_search, sp.subjects_search) AGAINST (? IN BOOLEAN MODE)` against `ft_summary_pubs_metadata` with per-token `+` requirements (same AND semantics as the works path). Filter-only paths (`language`, `year_from`/`year_to`, `type`, `open_access`, `peer_reviewed`, `has_files`, `venue_id`, `publisher_id`, `work_id`, `doi`) hit MariaDB B-tree indexes directly. The response always surfaces `meta.engine = "MariaDB"`. List rows join `venues v` and `organizations publisher` so `venue` (type / issn / eissn / scopus_id / wikidata_id / openalex_id) and `publisher` hydrate; `source`, `license_url`, `license_version` are surfaced on every list row. Detail responses embed `work`, `siblings[]`, `files[]`, optional `citations` / `references`.
...
### Venues endpoints
- `/venues`, `/venues/{id}`, `/venues/{id}/works`, `/venues/search`, `/venues/statistics` are backed by `summary_venues` (joined with `venues` + `organizations` for base columns and publisher). Subjects come from the embedded `top_subjects_json` (pre-sorted top 10). `/venues/search` runs a LIKE-based search across `summary_venues.name_search` / `abbrev_search` / `publisher_search` / `issn` / `eissn` ordered by `global_ranking_score`.
...
### Search endpoints
- `/search/works`, `/search/advanced`: `q` is optional; filter-only queries (e.g. `venue=mana`) are supported. The metadata-filter AND semantics described under [Works endpoints](#works-endpoints) apply (`+token1 +token2 …` BOOLEAN MODE against `ft_summary_pubs_metadata`). The `search:works:v2:*` cache key was bumped to `search:works:v3:*` when this contract changed so stale OR-mode payloads no longer get served from Redis.
- `/search/global`, `/search/persons`, `/search/autocomplete`, `/search/popular`, `/search/health`.
- `/search/autocomplete` honours the `{ suggestions, type, count, query, generated_at }` envelope. The service runs `MATCH(title_search, abstract_search) AGAINST (:q IN BOOLEAN MODE)` against `ft_summary_pubs_content` (without `ORDER BY` so it terminates early) and returns an empty suggestions list if that path fails — it never surfaces `{ error: … }` inside a `status: success` payload.
...
## Search Engine
- No Sphinx daemon, RT index, indexer process, or `sphinx*` runtime artefact. The full-text search path is `MATCH(...) AGAINST (... IN BOOLEAN MODE)` against MariaDB FULLTEXT indexes — primarily `ft_summary_pubs_content` (title + abstract) and `ft_summary_pubs_metadata` (authors + venue + subjects).
- The operator-side build / refresh procedures (`sp_orchestrate_all_summaries`, `sp_build_summary_publications`, `sp_refresh_summary_publications_for_work`) keep `summary_publications` and its FULLTEXT indexes consistent with the base tables.
