...
Academic bibliographic system API built with Node.js/Express, backed by MariaDB. Full-text search runs entirely against MariaDB FULLTEXT indexes on the base tables (`works`, `venues`, `persons`, `subjects`, `organizations`). There is no Sphinx daemon, no RT index, no external indexer, and no `summary_*` denormalized layer — every read goes straight to the entity that owns the data.
...
- Topology: 23 base tables, 0 views, 2 functions, 41 stored procedures, 5 triggers. **No `summary_*` table exists**: `summary_publications`, `summary_persons`, `summary_venues` were dissolved in `calls/2026-05-23_dissolve_all_summaries.md`. Search-side payload (`authors_search`, `subjects_search`) lives directly on `works`; the FULLTEXT contracts live on `works` (3 indexes) and `venues` (`ft_venues_search`).
- Schema files:
...
## Search Engine
- No Sphinx daemon, RT index, indexer process, or `sphinx*` runtime artefact. The full-text search path is `MATCH(...) AGAINST (... IN BOOLEAN MODE)` against MariaDB FULLTEXT indexes pinned on the base tables: `ft_works_content` (title + subjects), `ft_works_metadata` (authors + subjects), `ft_works_authors_content` (title + authors + subjects), `ft_venues_search` (name + abbreviated name), `ft_persons_names` (preferred + given + family name), `ft_subjects_term` (term), `ft_organizations_name` (name).
- The operator-side procedure `sp_refresh_work_search_fields(p_work_id)` keeps `works.authors_search` and `works.subjects_search` consistent with the source tables after mutations to `authorships` / `work_subjects` / `persons` / `subjects`. The FULLTEXT indexes update in-place with the row, so no API-side rebuild is required.
