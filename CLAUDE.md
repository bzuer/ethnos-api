...
- List `sortBy` accepts `id|name|type|impact_factor|works_count|h_index|cited_by_count|score|ranking`. Default is `score` (global ranking score) in `DESC` order so the most important venues surface first; numeric/ranking fields default to `DESC`, while `id`, `name`, and `type` default to `ASC`. `global_ranking_score` is always applied as the tiebreaker after the primary sort.
