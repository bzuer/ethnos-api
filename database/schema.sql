...
DELIMITER ;;
CREATE TRIGGER `trg_authorships_queue_sphinx_insert`
AFTER INSERT ON `authorships`
FOR EACH ROW
BEGIN
    INSERT INTO sphinx_queue (operation, work_id, status)
    VALUES ('UPDATE', NEW.work_id, 'pending')
    ON DUPLICATE KEY UPDATE status = 'pending', queued_at = CURRENT_TIMESTAMP;
END ;;
DELIMITER ;
DELIMITER ;;
CREATE TRIGGER `trg_authorships_queue_sphinx_update`
AFTER UPDATE ON `authorships`
FOR EACH ROW
BEGIN
    IF OLD.work_id <> NEW.work_id THEN
        INSERT INTO sphinx_queue (operation, work_id, status)
        VALUES ('UPDATE', OLD.work_id, 'pending')
        ON DUPLICATE KEY UPDATE status = 'pending', queued_at = CURRENT_TIMESTAMP;
    END IF;
...
DELIMITER ;
DELIMITER ;;
CREATE TRIGGER `trg_authorships_queue_sphinx_delete`
AFTER DELETE ON `authorships`
FOR EACH ROW
BEGIN
    INSERT INTO sphinx_queue (operation, work_id, status)
    VALUES ('UPDATE', OLD.work_id, 'pending')
...
) ENGINE=InnoDB AUTO_INCREMENT=9591173 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_uca1400_ai_ci;
/*!40101 SET character_set_client = @saved_cs_client */;
...
) ENGINE=InnoDB AUTO_INCREMENT=1527520 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_uca1400_ai_ci;
/*!40101 SET character_set_client = @saved_cs_client */;
...
) ENGINE=InnoDB AUTO_INCREMENT=4894733 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_uca1400_ai_ci;
/*!40101 SET character_set_client = @saved_cs_client */;
...
) ENGINE=InnoDB AUTO_INCREMENT=1112570615 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_uca1400_ai_ci;
/*!40101 SET character_set_client = @saved_cs_client */;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;
...
/*!50003 SET collation_connection  = @saved_col_connection */ ;
DELIMITER ;;
CREATE TRIGGER `trg_publications_queue_sphinx_insert`
AFTER INSERT ON `publications`
FOR EACH ROW
BEGIN
    INSERT INTO sphinx_queue (operation, work_id, status)
...
DELIMITER ;
DELIMITER ;;
CREATE TRIGGER `trg_publications_queue_sphinx_update`
...
DELIMITER ;
DELIMITER ;;
CREATE TRIGGER `trg_publications_queue_sphinx_delete`
...
) ENGINE=InnoDB AUTO_INCREMENT=27628697 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_uca1400_ai_ci;
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `sphinx_works_summary` (
  ...
  KEY `idx_open_access_peer_reviewed` (`open_access`,`peer_reviewed`),
  KEY `idx_network_metrics` (`citation_count`,`reference_count`,`cited_by_count`),
  KEY `idx_author_string_not_null` (`author_string`(1)),
  FULLTEXT KEY `ft_main_content` (`title`,`subtitle`,`abstract`),
  FULLTEXT KEY `ft_metadata` (`author_string`,`first_author_name`,`venue_name`,`venue_abbrev`,`publisher_name`),
...
) ENGINE=InnoDB AUTO_INCREMENT=1989592 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_uca1400_ai_ci;
/*!40101 SET character_set_client = @saved_cs_client */;
...
) ENGINE=InnoDB AUTO_INCREMENT=1043530 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_uca1400_ai_ci;
/*!40101 SET character_set_client = @saved_cs_client */;
...
) ENGINE=InnoDB AUTO_INCREMENT=20356314 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_uca1400_ai_ci;
/*!40101 SET character_set_client = @saved_cs_client */;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;
...
/*!50003 SET collation_connection  = @saved_col_connection */ ;
DELIMITER ;;
CREATE TRIGGER `trg_work_references_queue_sphinx_insert`
AFTER INSERT ON `work_references`
FOR EACH ROW
BEGIN
    INSERT INTO sphinx_queue (operation, work_id, status)
    VALUES ('UPDATE', NEW.citing_work_id, 'pending')
    ON DUPLICATE KEY UPDATE status = 'pending', queued_at = CURRENT_TIMESTAMP;

    IF NEW.cited_work_id IS NOT NULL THEN
        INSERT INTO sphinx_queue (operation, work_id, status)
        VALUES ('UPDATE', NEW.cited_work_id, 'pending')
        ON DUPLICATE KEY UPDATE status = 'pending', queued_at = CURRENT_TIMESTAMP;
    END IF;
END ;;
DELIMITER ;
DELIMITER ;;
CREATE TRIGGER `trg_work_references_queue_sphinx_update`
AFTER UPDATE ON `work_references`
FOR EACH ROW
BEGIN
    INSERT INTO sphinx_queue (operation, work_id, status)
    ...
    IF OLD.citing_work_id <> NEW.citing_work_id THEN
        INSERT INTO sphinx_queue (operation, work_id, status)
        VALUES ('UPDATE', OLD.citing_work_id, 'pending')
        ON DUPLICATE KEY UPDATE status = 'pending', queued_at = CURRENT_TIMESTAMP;
    END IF;
    ...
    IF OLD.cited_work_id IS NOT NULL AND (NEW.cited_work_id IS NULL OR OLD.cited_work_id <> NEW.cited_work_id) THEN
        INSERT INTO sphinx_queue (operation, work_id, status)
        VALUES ('UPDATE', OLD.cited_work_id, 'pending')
        ON DUPLICATE KEY UPDATE status = 'pending', queued_at = CURRENT_TIMESTAMP;
    END IF;
END ;;
DELIMITER ;
DELIMITER ;;
CREATE TRIGGER `trg_work_references_queue_sphinx_delete`
AFTER DELETE ON `work_references`
FOR EACH ROW
BEGIN
    INSERT INTO sphinx_queue (operation, work_id, status)
    ...
    IF OLD.cited_work_id IS NOT NULL THEN
        INSERT INTO sphinx_queue (operation, work_id, status)
        VALUES ('UPDATE', OLD.cited_work_id, 'pending')
        ON DUPLICATE KEY UPDATE status = 'pending', queued_at = CURRENT_TIMESTAMP;
    END IF;
...
/*!40101 SET character_set_client = @saved_cs_client */;
DELIMITER ;;
CREATE TRIGGER `trg_work_subjects_queue_sphinx_insert`
AFTER INSERT ON `work_subjects`
FOR EACH ROW
BEGIN
    INSERT INTO sphinx_queue (operation, work_id, status)
    VALUES ('UPDATE', NEW.work_id, 'pending')
...
DELIMITER ;
DELIMITER ;;
CREATE TRIGGER `trg_work_subjects_queue_sphinx_update`
AFTER UPDATE ON `work_subjects`
FOR EACH ROW
BEGIN
    IF OLD.work_id <> NEW.work_id THEN
        INSERT INTO sphinx_queue (operation, work_id, status)
...
DELIMITER ;
DELIMITER ;;
CREATE TRIGGER `trg_work_subjects_queue_sphinx_delete`
AFTER DELETE ON `work_subjects`
FOR EACH ROW
BEGIN
    INSERT INTO sphinx_queue (operation, work_id, status)
    VALUES ('UPDATE', OLD.work_id, 'pending')
...
) ENGINE=InnoDB AUTO_INCREMENT=7471754 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_uca1400_ai_ci;
/*!40101 SET character_set_client = @saved_cs_client */;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;
...
/*!50106 CREATE*/ /*!50117 DEFINER=`pc`@`localhost`*/ /*!50106 EVENT `ev_process_sphinx_safe` ON SCHEDULE EVERY 1 MINUTE STARTS '2026-02-19 10:56:55' ON COMPLETION NOT PRESERVE ENABLE DO BEGIN
    ...
    SELECT
        ...
        COALESCE(lp.open_access, 0), COALESCE(lp.peer_reviewed, 1),
...
/*!50003 SET collation_connection  = utf8mb4_uca1400_ai_ci */ ;
DELIMITER ;;
CREATE DEFINER=`pc`@`localhost` PROCEDURE `sp_apply_structural_indexes`()
BEGIN
...
/*!50003 SET collation_connection  = utf8mb4_uca1400_ai_ci */ ;
DELIMITER ;;
CREATE DEFINER=`pc`@`localhost` PROCEDURE `sp_deduplicate_works_advanced`(IN p_batch_size INT)
BEGIN
...
/*!50003 SET collation_connection  = utf8mb4_uca1400_ai_ci */ ;
DELIMITER ;;
CREATE DEFINER=`pc`@`localhost` PROCEDURE `sp_fix_merged_work`(IN p_work_id INT)
BEGIN
...
/*!50003 SET collation_connection  = utf8mb4_uca1400_ai_ci */ ;
DELIMITER ;;
CREATE DEFINER=`pc`@`localhost` PROCEDURE `sp_merge_single_venue_pair`(
...
/*!50003 SET collation_connection  = utf8mb4_uca1400_ai_ci */ ;
DELIMITER ;;
CREATE DEFINER=`pc`@`localhost` PROCEDURE `sp_normalize_publications_data`()
BEGIN
...
/*!50003 SET collation_connection  = utf8mb4_uca1400_ai_ci */ ;
DELIMITER ;;
CREATE DEFINER=`pc`@`localhost` PROCEDURE `sp_refresh_collaboration_cache`(IN p_apply TINYINT, IN p_min_collaborations INT)
BEGIN
...
/*!50003 SET collation_connection  = utf8mb4_uca1400_ai_ci */ ;
DELIMITER ;;
CREATE DEFINER=`pc`@`localhost` PROCEDURE `sp_repair_work_references_consistency`(IN p_apply TINYINT)
BEGIN
    DECLARE v_apply TINYINT DEFAULT 1;
...
/*!50003 SET collation_connection  = utf8mb4_uca1400_ai_ci */ ;
DELIMITER ;;
CREATE DEFINER=`pc`@`localhost` PROCEDURE `sp_run_pending_venue_merges`()
BEGIN
    DELETE p_del
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`pc`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `v_institution_productivity_optimized` AS select `o`.`id` AS `id`,`o`.`name` AS `institution_name`,`o`.`type` AS `type`,`o`.`country_code` AS `country_code`,count(distinct `a`.`work_id`) AS `total_works`,count(distinct `a`.`person_id`) AS `unique_researchers`,count(distinct case when `open_works`.`is_oa` = 1 then `a`.`work_id` end) AS `open_access_works`,coalesce(sum(`distinct_works`.`cit`),0) AS `total_citations`,min(`py`.`min_yr`) AS `first_publication_year`,max(`py`.`max_yr`) AS `latest_publication_year` from ((((`organizations` `o` join `authorships` `a` on(`o`.`id` = `a`.`affiliation_id`)) join (select `data_dev`.`works`.`id` AS `id`,`data_dev`.`works`.`citation_count` AS `cit` from `works`) `distinct_works` on(`a`.`work_id` = `distinct_works`.`id`)) join (select `data_dev`.`publications`.`work_id` AS `work_id`,min(`data_dev`.`publications`.`year`) AS `min_yr`,max(`data_dev`.`publications`.`year`) AS `max_yr` from `publications` group by `data_dev`.`publications`.`work_id` having min(`data_dev`.`publications`.`year`) is not null) `py` on(`a`.`work_id` = `py`.`work_id`)) left join (select `data_dev`.`publications`.`work_id` AS `work_id`,max(`data_dev`.`publications`.`open_access`) AS `is_oa` from `publications` group by `data_dev`.`publications`.`work_id`) `open_works` on(`a`.`work_id` = `open_works`.`work_id`)) where `a`.`affiliation_id` is not null group by `o`.`id`,`o`.`name`,`o`.`type`,`o`.`country_code` */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
-- Dump completed on 2026-03-02 17:39:12
