#!/usr/bin/env bash
#
# Render config/nginx.conf into the live nginx vhost that fronts the API, taking
# the port topology from /etc/node-backend.env. The API process listens on
# loopback only; this config is what makes the public API port reachable, so the
# API is never published except through nginx.
#
...
DEST="${NGINX_API_CONF:-/etc/nginx/conf.d/ethnos-api.conf}"
...
TLS_PORT="${NGINX_TLS_PORT:-${HTTPS_PORT:-3443}}"
SSL_CERT="${NGINX_SSL_CERT:-}"
SSL_KEY="${NGINX_SSL_KEY:-}"
BODY_SIZE="${NGINX_CLIENT_MAX_BODY_SIZE:-10m}"
PROXY_TIMEOUT="${NGINX_PROXY_TIMEOUT:-30s}"

if [ "$UPSTREAM_PORT" = "$PUBLIC_PORT" ]; then
  echo "PORT ($UPSTREAM_PORT) must differ from NGINX_PUBLIC_PORT ($PUBLIC_PORT):" \
...
case "$UPSTREAM_HOST" in
  127.*|::1|localhost) ;;
  *) echo "API_BIND_HOST is '$UPSTREAM_HOST' — the application must bind loopback" \
