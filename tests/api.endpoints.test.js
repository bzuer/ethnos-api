...
const realTimeIndexingService = require('../src/services/realTimeIndexing.service');
...
describe('Real-time indexing service', () => {
  test('indexNewPublication refreshes summary and pushes the row to publications_rt', async () => {
    const calls = { proc: [], sphinxIndex: [], cacheDel: [] };

    stubMethod(sequelize, 'query', async (sql, options) => {
      const text = String(sql || '');
      if (text.includes('FROM publications WHERE id')) {
        return [{ work_id: 7 }];
      }
      if (text.includes('CALL sp_refresh_summary_publications_for_work')) {
        calls.proc.push(options?.replacements?.[0]);
        return [[], 0];
      }
      if (text.includes('FROM summary_publications')) {
        return [
          {
            publication_id: 42,
            work_id: 7,
            venue_id: 3,
            publisher_id: null,
            title_search: 'Refreshed title',
            abstract_search: '',
            authors_search: 'Alice Bob',
            venue_search: 'Mana',
            subjects_search: '',
            doi: '10.5/x',
            work_type: 'ARTICLE',
            ...
            peer_reviewed: 1,
            has_files: 1,
            work_citation_count: 5,
            work_reference_count: 12,
            publication_download_count: 3
          }
        ];
      }
      return [];
    ...
    stubMethod(sphinxService, 'ensureConnection', async () => true);
    stubMethod(sphinxService, 'indexPublication', async (row) => {
      calls.sphinxIndex.push(row);
      return { affectedRows: 1 };
    });
    stubMethod(cacheService, 'del', async (key) => {
      calls.cacheDel.push(key);
      return true;
    });

    const wasEnabled = realTimeIndexingService.enabled;
    realTimeIndexingService.enabled = true;
    try {
      const result = await realTimeIndexingService.indexNewPublication(42);
      expect(result.success).toBe(true);
      expect(result.work_id).toBe(7);
      expect(calls.proc[0]).toBe(7);
      expect(calls.sphinxIndex).toHaveLength(1);
      expect(calls.sphinxIndex[0]).toHaveProperty('publication_id', 42);
      expect(calls.sphinxIndex[0]).toHaveProperty('has_files', 1);
      expect(calls.sphinxIndex[0]).toHaveProperty('publication_download_count', 3);
      expect(calls.cacheDel.some(key => key.startsWith('work:v2:7:'))).toBe(true);
      expect(calls.cacheDel.some(key => key.startsWith('publication:42:v1:'))).toBe(true);
    } finally {
      realTimeIndexingService.enabled = wasEnabled;
    }
  });

  test('indexNewWork delegates to rebuildPublicationsForWork and reindexes every sibling', async () => {
    const calls = { proc: [], sphinxIndex: [] };
    ...
    stubMethod(sequelize, 'query', async (sql, options) => {
      ...
      if (text.includes('FROM summary_publications sp')) {
        return [
          {
            publication_id: 100,
            work_id: 7,
            ...
            peer_reviewed: 1,
            has_files: 0,
            work_citation_count: 1,
            work_reference_count: 0,
            publication_download_count: 0
          },
          {
            publication_id: 101,
            work_id: 7,
    ...
    stubMethod(sphinxService, 'ensureConnection', async () => true);
    stubMethod(sphinxService, 'indexPublication', async (row) => {
      calls.sphinxIndex.push(row.publication_id);
      return { affectedRows: 1 };
    });
    stubMethod(cacheService, 'del', async () => true);

    const wasEnabled = realTimeIndexingService.enabled;
    realTimeIndexingService.enabled = true;
    try {
      const result = await realTimeIndexingService.indexNewWork(7);
      expect(result.success).toBe(true);
      expect(result.indexed).toBe(2);
      expect(calls.proc[0]).toBe(7);
      expect(calls.sphinxIndex).toEqual([100, 101]);
    } finally {
      realTimeIndexingService.enabled = wasEnabled;
    }
  });

  test('deletePublication tears down sphinx_rt entry and refreshes the surviving siblings', async () => {
    const calls = { sphinxDelete: [], proc: [], cacheDel: [] };
    ...
    stubMethod(sphinxService, 'ensureConnection', async () => true);
    stubMethod(sphinxService, 'deletePublication', async (id) => {
      calls.sphinxDelete.push(id);
      return { affectedRows: 1 };
    });
    stubMethod(cacheService, 'del', async (key) => {
      calls.cacheDel.push(key);
    ...
    realTimeIndexingService.enabled = true;
    try {
      const result = await realTimeIndexingService.deletePublication(42);
      expect(result.success).toBe(true);
      expect(result.work_id).toBe(7);
      expect(calls.sphinxDelete).toEqual([42]);
      expect(calls.proc[0]).toBe(7);
      expect(calls.cacheDel.some(key => key.startsWith('publication:42:v1:'))).toBe(true);
      expect(calls.cacheDel.some(key => key.startsWith('work:v2:7:'))).toBe(true);
    } finally {
